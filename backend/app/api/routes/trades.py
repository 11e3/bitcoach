...
def _parse_order(order: dict) -> dict:
    ...
    Uses executed_funds when available, otherwise price × volume.
    Market sells with price=null get funds=0 (excluded from analysis).
    """
    side = "buy" if order.get("side") == "bid" else "sell"
    volume = float(order.get("executed_volume", 0) or 0)
    fee = float(order.get("paid_fee", 0) or 0)

    executed_funds = float(order.get("executed_funds", 0) or 0)
    if executed_funds > 0:
...
async def sync_trades(db: AsyncSession = Depends(get_db), session_token: str = Depends(get_session_token)):
    ...
    await db.flush()
    return SyncResponse(synced=synced, skipped=skipped, total=synced + skipped)
